"""Supabase shared_learnings importer — SPEC Section 7.8.

Reads from the Supabase shared_learnings table and ingests as Mycelium facts.
Migration order: Supabase first (already structured, cleanest mapping).

Requires: asyncpg connection to Supabase Postgres (direct connection, not REST API).
"""

from __future__ import annotations

from datetime import datetime
from typing import TYPE_CHECKING, Any, Protocol, cast

from mycelium.domain.types import FactContent, SourceType
from mycelium.migration.base import (
    MIGRATION_CONFIDENCE,
    MigrationRecord,
    MigrationResult,
    MigrationSource,
)

if TYPE_CHECKING:
    from mycelium.client.client import MyceliumClient
    from mycelium.ops.logger import OpsLogger


class _RowLike(Protocol):
    def __getitem__(self, key: str) -> object: ...
    def get(self, key: str, default: object | None = None) -> object | None: ...


class _SupabaseConn(Protocol):
    async def fetch(self, query: str) -> list[_RowLike]: ...


def _as_str(value: object | None, default: str) -> str:
    if isinstance(value, str) and value.strip():
        return value
    if value is None:
        return default
    rendered = str(value).strip()
    return rendered if rendered else default


def _as_optional_str(value: object | None) -> str | None:
    if isinstance(value, str):
        return value
    if value is None:
        return None
    return str(value)


def _as_tags(value: object | None) -> list[str]:
    if value is None:
        return []
    if isinstance(value, list):
        parsed: list[str] = []
        for raw_tag in cast("list[object]", value):
            rendered = str(raw_tag).strip()
            if rendered:
                parsed.append(rendered)
        return parsed
    rendered = str(value).strip()
    return [rendered] if rendered else []


# Default query — adjust columns based on actual shared_learnings schema
_DEFAULT_QUERY = """
    SELECT id, agent_id, subject, predicate, content, context, tags, created_at
    FROM shared_learnings
    ORDER BY created_at ASC
"""


async def extract_from_supabase(
    conn: _SupabaseConn,
    query: str = _DEFAULT_QUERY,
) -> list[MigrationRecord]:
    """Extract records from Supabase shared_learnings table.

    Args:
        conn: asyncpg connection to Supabase Postgres.
        query: SQL query to extract records (override for custom schemas).

    Returns:
        List of MigrationRecord ready for ingest.
    """
    rows = await conn.fetch(query)

    records: list[MigrationRecord] = []
    for row in rows:
        subject = _as_str(cast("Any", row["subject"]), "unknown")
        predicate = _as_str(cast("Any", row.get("predicate")), "has_learning")
        obj = _as_str(cast("Any", row["content"]), "")
        if not obj:
            continue
        record = MigrationRecord(
            subject=subject,
            predicate=predicate,
            object=obj,
            context=_as_optional_str(row.get("context")),
            tags=_as_tags(row.get("tags")),
            source_agent_id=_as_optional_str(row.get("agent_id")),
            original_id=str(row["id"]),
            original_created_at=cast("datetime | None", row.get("created_at")),
        )
        records.append(record)

    return records


async def import_supabase_learnings(
    records: list[MigrationRecord],
    client: MyceliumClient,
    ops_logger: OpsLogger | None = None,
    default_agent_id: str = "migration-agent",
) -> MigrationResult:
    """Ingest extracted Supabase records into Mycelium.

    Args:
        records: Pre-extracted migration records.
        client: Connected MyceliumClient to ingest through.
        ops_logger: Optional ops logger for migration tracking.
        default_agent_id: Fallback agent_id if record doesn't specify one.

    Returns:
        MigrationResult with counts and any errors.
    """
    result = MigrationResult(
        source=MigrationSource.SUPABASE,
        total_records=len(records),
    )

    for record in records:
        try:
            content = FactContent(
                subject=record.subject,
                predicate=record.predicate,
                object=record.object,
                context=record.context,
            )

            ingest_result = await client.ingest(
                content=content,
                source_type=SourceType.AGENT_EXTRACTION,
                tags=record.tags,
                initial_confidence=MIGRATION_CONFIDENCE,
                metadata={
                    "migrated_from": MigrationSource.SUPABASE.value,
                    "migration_date": datetime.now().isoformat(),
                    "original_id": record.original_id,
                },
            )

            if ingest_result.accepted:
                result.ingested += 1
            else:
                result.skipped += 1
                reason = str(ingest_result.rejection) if ingest_result.rejection else "unknown"
                result.errors.append(
                    f"Record {record.original_id} skipped: {reason}"
                )

        except Exception as e:
            result.failed += 1
            result.errors.append(
                f"Record {record.original_id} failed: {e}"
            )

    result.finished_at = datetime.now()

    if ops_logger is not None:
        await ops_logger.log(
            "migration", "completed",
            detail={
                "source": MigrationSource.SUPABASE.value,
                "total": result.total_records,
                "ingested": result.ingested,
                "skipped": result.skipped,
                "failed": result.failed,
                "success_rate": result.success_rate,
            },
        )

    return result
